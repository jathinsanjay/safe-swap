...
const Upload =  () => {
  const [isLoading, setLoading] = useState(false);
    ...
    const showToastErrorMessage = () => {
      toast.warning("This purchase ID has already been used", {
        position: toast.POSITION.TOP_RIGHT,
      });
    }

      const showToastInvalidMessage = () => {
        toast.error("Your Id is Invalid", {
          position: toast.POSITION.TOP_RIGHT,
        });
      }
   ...
   let response2;
    ...
    //  let usedPurchaseIds = ["0x4c3f2ea3c6b6b819f9bfe550f7b8c43087ce84a6fa6c194edcc0f9b0ffb30eee"];
     ...
     const sellercontract = new web3.eth.Contract(abii,"0x93140112610128D738b264E1a913CC2142548BfB");
    ...
    //  setResult(r);
     await sellercontract.methods.verify(purchaseId).call().then(result => {
      ...
      if(usedPurchaseIds.includes(purchaseId)){
        ...
        contract.methods.addPurchase(purchaseId,username,purchasePrice,purchaseDate,sellerName,purchaserName,response1.data.IpfsHash,response2.data.IpfsHash,response3.data.IpfsHash,response4.data.IpfsHash,val).send({from : acc[0]})
